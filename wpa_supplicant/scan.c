...
int wpas_channel_width_offset(enum chan_width cw)
{
	switch (cw) {
	case CHAN_WIDTH_40:
		return 1;
 ...
 * based on the beacon RSSI at the STA. Beacons are transmitted on 20 MHz
 ...
 * power limit of the STA defined by the transmit power envelope IE and the tx
 * power of the AP, the difference in the tx power of X MHz and Y MHz at the STA
 * equals to the difference at the AP, and the difference in the signal RSSI at
 * the STA. tx_pwr is a floating point number in the spec, but the error of
 * casting to int is trivial in comparing two BSSes.
 */
static int wpas_channel_width_tx_pwr(const u8 *ies,
				     size_t ies_len,
				     enum chan_width cw)
{
#define MIN(a, b) (a < b ? a : b)
	...
	for_each_element_id(elem, WLAN_EID_TRANSMIT_POWER_ENVELOPE, ies,
			    ies_len) {
		/*
		 * 80211ax-2021 9.4.2.161 Transmit Power Envelope element
		 * defines Maximum Transmit Power Count (B0-B2), Maximum
		 * Transmit Power Interpretation (B3-B5) and Maximum Transmit
		 * Power Category (B6-B7).
		 */
		int max_tx_pwr_count = elem->data[0] & 0x07;
		...
		if (tx_pwr_intrpn == LOCAL_EIRP ||
			...
			tx_pwr = (signed char)elem->data[MIN(offset,
							 max_tx_pwr_count) + 1];
			/*
