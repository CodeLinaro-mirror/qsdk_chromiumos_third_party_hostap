...
static int wpa_config_set_cred_ois(u8 cred_ois[MAX_ROAMING_CONS][MAX_ROAMING_CONS_OI_LEN],
			...
			const char *value, int line)
{
	...
	if (os_strcmp(var, "roaming_consortium") == 0) {
		...
		wpa_printf(MSG_WARNING, "Line %d: option roaming_consortium is "
	...
	if (os_strcmp(var, "required_roaming_consortium") == 0) {
		...
		wpa_printf(MSG_WARNING,
			   "Line %d: option required_roaming_consortium is "
			   "deprecated and will be removed in the future",
			   line);
	...
	if (os_strcmp(var, "required_home_ois") == 0) {
		...
		if (res < 0)
			wpa_printf(MSG_ERROR,
				   "Line %d: invalid required_home_ois",
				   line);
		os_free(val);
		return res;
	}
